...
with open('day25/input.txt') as file:
    ...
    card_key = int(raw_data[0].strip())
    door_key = int(raw_data[1].strip())


def find_loops(subject_number,public_key):
    loops = 0
...
def create_key(loops,subject_number):
    value = 1
...
card_loops = find_loops(7,card_key)
door_loops = find_loops(7,door_key)
print(f"Card loop size: {card_loops} \nDoor loop size: {door_loops}")



encrption_key = create_key(card_loops,door_key)
print(f"Encryption key: {encrption_key}")