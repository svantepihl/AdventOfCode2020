...
def parse_rules(raw_rules):
    ...
    for line in raw_rules.splitlines():
        ruleNum, options = line.split(': ')
        rules[int(ruleNum)] = [option.split()
...
def generate_options(ruleNum, rules):
    ruleOptions = rules[ruleNum]
    if ['"a"'] in ruleOptions:
        return ['a']
    ...
    if ruleNum in procssedRules:
        return procssedRules[ruleNum]

    all_options = []
    for option in ruleOptions:
        options = []
        for rule in option:
            sub_options = generate_options(int(rule), rules)
            ...
            if len(options) == 0:
                ...
                for s in sub_options:
                    for op in options:
                        combined_options.append(op+s)
                options = combined_options.copy()
        all_options += options

    procssedRules[ruleNum] = all_options
    return all_options


def count_matches(options_list, messages):
    options_set = set(options_list)

    count = 0

    for message in messages:
        if message in options_set:
            count += 1
    return count
...
rules_1 = parse_rules(raw_rules)

# Part 1
print('Part 1: ' + str(count_matches(generate_options(0, rules_1), messages)))


# Part 2
def part_2():
    rule42 = procssedRules[42]
    rule31 = procssedRules[31]
    chunkSize = len(rule42[0])

    count = 0
    for message in messages:
        chunks42 = [False for _ in range(len(message)//chunkSize)]
        chunks31 = [False for _ in range(len(message)//chunkSize)]

        currChunk = 0
        for i in range(0, len(message), chunkSize):
            if message[i:i+chunkSize] in rule42:
                chunks42[currChunk] = True
            if message[i:i+chunkSize] in rule31:
